...
# compared with original code, there are some changes
# terminal is used when update Q values
# num_episodes = 1200, capacity = 400, batch_size = 64
# add code to plot rewards of episodes
...
class DQN():
    ...
    def update(self):
        if self.memory_count >= self.capacity:
            ...
            terminal = torch.tensor([t.terminal for t in self.memory]).float()

            reward = (reward - reward.mean()) / (reward.std() + 1e-7)
            with torch.no_grad():
                target_v = reward + terminal * self.gamma * self.target_net(next_state).max(1)[0]
