    ...
    def update(self, i_ep):
        ...
        for i in range(self.ppo_update_time):
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer))), self.batch_size, False):
                ...
                V = self.critic_net(state[index]) # V will change inside this update loop as the critic is optimized later
                delta = Gt_index - V
                advantage = delta.detach()
                # epoch iteration, PPO core!!!
                action_prob = self.actor_net(state[index]).gather(1, action[index])  # new policy

                ratio = (action_prob / old_action_prob[index]) # "ratio" at the beginning is 1, but after actor update, it will no longer be 1
                ...
                value_loss = F.mse_loss(Gt_index, V) #Gt are static targets
